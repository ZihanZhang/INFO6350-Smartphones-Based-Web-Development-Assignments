...
//            }
//        }
        performSegue(withIdentifier: "newsegue2", sender: self)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "newsegue2" {
            if let toViewController = segue.destination as? detailViewController {
                toViewController.curItem = curPurchase.items[(llistView.indexPathForSelectedRow?.row)!]
            }
        }
    }
